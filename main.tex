...
\begin{document}
	...
	\section{Coherent transmission system}
	...
	\begin{frame}{Overview}
		\begin{figure}
			\begin{tikzpicture}[
				node distance=12mm,
				arrow/.style={very thick, -Latex},
				block/.style={draw, very thick, minimum height=12mm, minimum width=28mm, align=center},
			]
				\node[block] (source) {Information\\source};
				\node[block, above=of source] (transmitter) {Transmitter};
				\node[block, right=26mm of transmitter] (channel) {Channel};
				\node[block, right=26mm of channel] (receiver) {Receiver};
				\node[block, below=of receiver] (destination) {Information\\destination}; 
				
				\draw[arrow] (source) -- (transmitter) node[midway, left] {$\left\{\alpha_n\right\}_{n\in I}$};
				\draw[arrow] (transmitter) -- (channel) node[midway, above] {$\Re\left\{\alpha(t)e^{-i\omega_c t}\right\}$};
				\draw[arrow] (channel) -- (receiver) node[midway, above] {$\Re\left\{\beta(t)e^{-i\omega_c t}\right\}$};
				\draw[arrow] (receiver) -- (destination) node[midway, right] {$\left\{\beta_n\right\}_{n\in I}$};;
			\end{tikzpicture}
			\caption{Coherent transmission system correlating an information destination with an information source.}
		\end{figure}
	\end{frame}
	
	\begin{frame}{Optical transmitter setups}
		\begin{figure}
			\centering
			\includegraphics{figures/coherent-transmitter}
			\caption{foo}
		\end{figure}
	\end{frame}
	
	\begin{frame}{Optical receiver setups}
		\begin{figure}
			\centering
			\includegraphics{figures/coherent-receiver-active}
			\caption{foo}
		\end{figure}
	\end{frame}
